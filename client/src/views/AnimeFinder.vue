...
export default {
  ...
  computed: {
    ...mapState(useAnimeStore, ['animeSearchResult']),
  }
}
  ...
  <div class="bg-gray-900 px-4 py-16 mx-auto sm:max-w-xl md:max-w-full lg:max-w-screen-xl md:px-24 lg:px-8 lg:py-4">
    <div class="flex flex-col lg:flex-row">
      <div class="w-1/2  mx-auto ">
              ...
              <div class="flex space-x-4">
                <div class="flex rounded-md overflow-hidden w-full">
                  <input v-model="inputSearch" type="text"
                    class="bg-white font-bold px-2 text-xl w-full rounded-md rounded-r-none" />
                  <button type="submit"
                    class="bg-indigo-600 text-white px-6 text-lg font-semibold py-1 rounded-r-md">Go</button>
                </div>
                <button @click.prevent="inputSearch = ''"
                  class="bg-white px-2 text-sm font-semibold py-1 rounded-md">Clear</button>
              </div>
            </div>
          </form>
        </div>
        <div>
          <img v-if="imageDetail == ''" class="object-cover w-full  rounded shadow-lg " src="../assets/confused.jpg"
            alt="" />
          <img v-if="imageDetail !== ''" class="object-cover w-full  rounded shadow-lg " :src="imageDetail" alt="" />
        </div>
        <!-- Base -->

        <button v-if="imageDetail !== ''" @click.prevent="submitCreatePlaylist"
          class="w-full group flex items-center justify-between rounded-lg border border-green-600 bg-green-500 px-5 py-3 transition-colors hover:bg-green-700 focus:outline-none focus:ring">
          <span class="font-medium text-white transition-colors ">
            Add to playlist
          </span>

          <span class="ml-4 flex-shrink-0 rounded-full  bg-white p-2 text-indigo-600 group-active:text-indigo-500">
        ...
        <div class="px-4 text-white mt-7 w-full">
          <div class=" gap-12 row-gap-8 lg:grid-cols-2">
            <div class="flex flex-col justify-center">
              <div class=" max-w-xl mb-1">
                <h2 class="text-center mb-7 max-w-lg   font-sans text-3xl font-bold tracking-tight  sm:text-4xl sm:leading-none">
                  {{ titleDetail }}<br class="hidden md:block" />
                </h2>
                <div v-if="imageDetail !== ''" class="mt-3 mb-6 flex gap-5 items-center justify-center">
      ...
      <div class=" w-1/2 ml-5 pt-16 sm:grid-cols-2">
        <article v-for="data in animeSearchResult" :key="data.id"
          ...
          <a @click.prevent="animeIdDetail = data._id, titleDetail = data.title, synopsisDetail = data.synopsis, imageDetail = data.image, episodesDetail = data.episodes, statusDetail = data.status"
            href="">
            <div @click.prevent="scrollToTop">
                ...
                <div class="ml-4">
                  ...
                  <div class="mt-2 sm:flex sm:items-center sm:gap-2">
                    <div class="flex items-center">
                      ...
                      <p class="ml-1 text-xs">{{ data.episodes }} episodes</p>
                    </div>
                    ...
                    <p class="hidden sm:block sm:text-xs sm:text-gray-500">
                      Genre
                      <span v-for="genre in data.genres" class="font-medium  ">
                        {{ genre }} &nbsp
                      </span>
                    </p>
                  </div>
              ...
              <div class="flex justify-end">
                <strong
                  ...
                  <span class="text-[10px] font-medium sm:text-xs">{{ data.status }}</span>
                </strong>
              </div>
            </div>
          </a>
