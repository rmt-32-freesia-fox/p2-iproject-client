...
export default {
  ...
  computed: mapState(useProfileStore, ['isMe']),
  methods: {
    ...mapActions(useLinkStore, ['remove']),
    toggle() {
      this.show = !this.show
    },
  },
...
<template>
  ...
  >
    <div class="bg-black rounded-lg p-10 flex flex-col gap-3">
      <div class="text-xl">Are you sure want to remove this link?</div>
      <br />
      <div class="flex gap-1">
        <button @click="toggle" class="btn">Cancel</button>
        <button @click="remove(id)" class="btn">Remove</button>
      </div>
    </div>
  </div>
